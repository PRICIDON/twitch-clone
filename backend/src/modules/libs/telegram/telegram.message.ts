...
export const MESSAGES = {
  welcome:
    `<b>👋 Добро пожаловать в PricidonStream Bot!</b>\n\n` +
    `Чтобы получать уведомления и улучшить ваш опыт использования платформы, давайте свяжем ваш Telegram аккаунт с PricidonStream.\n\n` +
    `Нажмите на кнопку ниже и перейдите в раздел <b>Уведомления</b>, чтобы завершить настройку.`,
  authSuccess: `🎉 Вы успешно авторизовались и Telegram аккаунт связан с PricidonStream!\n\n`,
  ...
  follows: (user: User) =>
    `📺 <a href="https://pricidon.ru/${user.username}">${user.username}</a>`,
  resetPassword: (token: string, metadata: SessionMetadata) =>
    `<b>🔒 Сброс пароля</b>\n\n` +
    `Вы запросили сброс пароля для вашей учетной записи на платформе <b>PricidonStream</b>.\n\n` +
    `Чтобы создать новый пароль, пожалуйста, перейдите по следующей ссылке:\n\n` +
    `<b><a href="https://pricidon.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
    ...
    `Спасибо за использование <b>PricidonStream</b>! 🚀`,
  deactivate: (token: string, metadata: SessionMetadata) =>
    `<b>⚠️ Запрос на деактивацию аккаунта</b>\n\n` +
    `Вы инициировали процесс деактивации вашего аккаунта на платформе <b>Pricidonstream</b>.\n\n` +
    ...
    `Спасибо, что пользуетесь <b>PricidonStream</b>! Мы всегда рады видеть вас на нашей платформе и надеемся, что вы останетесь с нами. 🚀\n\n` +
    `С уважением,\n` +
    `Команда PricidonStream`,
  accountDeleted:
    `<b>⚠️ Ваш аккаунт был полностью удалён.</b>\n\n` +
    `Ваш аккаунт был полностью стерт из базы данных Pricidonstream. Все ваши данные и информация были удалены безвозвратно. ❌\n\n` +
    ...
    `<b><a href="https://pricidon.ru/account/create">Зарегистрироваться на Pricidonstream</a></b>\n\n` +
  ...
  verifyChannel:
    ...
    `Спасибо, что вы с нами и продолжаете развивать свой канал вместе с PricidonStream!`,
};
