...
export default function SessionItem({session, isCurrentSession} : SessionItemProps) {
	...
	return <CardContainer
		...
		Icon={getBrowserIcon(session.metadata.device.browser)}
		rightContent={<div className="flex items-center gap-x-4">
			{!isCurrentSession && (
				...
				>
					<Button disabled={isLoadingRemove} variant="secondary">{t('deleteButton')}</Button>
				</ConfirmModal>
			)}
