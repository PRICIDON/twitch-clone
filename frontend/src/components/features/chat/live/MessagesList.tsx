...
import {
	...
	FindChatMessagesByStreamQuery, useChatMessageAddedSubscription,
	useFindChatMessagesByStreamQuery, useFindSponsorsByChannelQuery
 	...
 	return (
		<div className='flex h-full flex-1 flex-col-reverse'>
			{messages.map((message, index) => (
				<MessageItem data={message} key={index}  isSponsor={sponsorIds.has(message.user.id)} />
			))}
		</div>
	)
}
